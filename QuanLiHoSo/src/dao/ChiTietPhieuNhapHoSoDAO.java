...
public class ChiTietPhieuNhapHoSoDAO implements DAOInterface<ChiTietPhieu> {

    public static ChiTietPhieuNhapHoSoDAO getInstance() {
        return new ChiTietPhieuNhapHoSoDAO();
    }
    ...
    public int insert(ChiTietPhieu t) {
        ...
        try {
            ...
            String sql = "INSERT INTO ChiTietPhieuNhapHoSo (maPhieu, maHoSo, soLuong, donGia) VALUES (?,?,?,?)";
    ...
    public int update(ChiTietPhieu t) {
        ...
        try {
            Connection con = JDBCUtil.getConnection();
            String sql = "UPDATE ChiTietPhieuNhapHoSo SET maPhieu=?, maHoSo=?, soLuong=?, donGia = ?  WHERE maPhieu=? AND maHoSo=?";
            PreparedStatement pst = con.prepareStatement(sql);
            pst.setString(1, t.getMaPhieu());
            pst.setString(2, t.getMaHoSo());
            pst.setInt(3, t.getSoLuong());
    ...
    public int delete(ChiTietPhieu t) {
        ...
        try {
            Connection con = JDBCUtil.getConnection();
            String sql = "DELETE FROM ChiTietPhieuNhapHoSo WHERE maPhieu=?";
            PreparedStatement pst = con.prepareStatement(sql);
            pst.setString(1, t.getMaPhieu());
            ketQua = pst.executeUpdate();
            JDBCUtil.closeConnection(con);
    ...
    public ArrayList<ChiTietPhieu> selectAll(String t) {
        ...
        try {
            Connection con = JDBCUtil.getConnection();
            String sql = "SELECT * FROM ChiTietPhieuNhapHoSo WHERE maPhieu=?";
            PreparedStatement pst = con.prepareStatement(sql);
            pst.setString(1, t);
    ...
    public ArrayList<ChiTietPhieu> selectAll() {
        ...
        try {
            Connection con = JDBCUtil.getConnection();
            String sql = "SELECT * FROM ChiTietPhieuNhapHoSo";
            PreparedStatement pst = con.prepareStatement(sql);
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                String maPhieu = rs.getString("maPhieu");
