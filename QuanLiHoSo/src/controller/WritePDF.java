...
import dao.ChiTietPhieuNhapHoSoDAO;
import dao.ChiTietPhieuXuatHoSoDAO;
import dao.HoSoDAO;
import dao.NhanVienDAO;
import dao.PhieuNhapHoSoDAO;
import dao.PhieuXuatHoSoDAO;
...
public class WritePDF {
    ...
    public void writePhieuNhap(String mapn) {
        ...
        try {
            ...
            PhieuNhap pn = PhieuNhapHoSoDAO.getInstance().selectById(mapn);
            ...
            para2.add(String.valueOf("\nNhà cung cấp: " + NhanVienDAO.getInstance().selectById(pn.getNhanVien()).getTenNhanVien() + "  -  " + pn.getNhanVien()));
            ...
            for (ChiTietPhieu ctpn : ChiTietPhieuNhapHoSoDAO.getInstance().selectAll(mapn)) {
                HoSo mt = HoSoDAO.getInstance().selectById(ctpn.getMaHoSo());
                pdfTable.addCell(new PdfPCell(new Phrase(ctpn.getMaHoSo(), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(mt.getTenNguoi(), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(formatter.format(mt.getCMND()) + "đ", fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(String.valueOf(ctpn.getSoLuong()), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(formatter.format(ctpn.getSoLuong() * mt.getCMND()) + "đ", fontData)));
            }

            document.add(pdfTable);
            document.add(Chunk.NEWLINE);

            Paragraph paraTongThanhToan = new Paragraph(new Phrase("Tổng thanh toán: " + formatter.format(pn.getTongThoiGianLamViec()) + "đ", fontData));
    ...
    public void writePhieuXuat(String mapn) {
        ...
        try {
            ...
            PhieuXuat pn = PhieuXuatHoSoDAO.getInstance().selectById(mapn);
            ...
            //Truyen thong tin tung chi tiet vao table
            for (ChiTietPhieu ctpn : ChiTietPhieuXuatHoSoDAO.getInstance().selectAll(mapn)) {
                HoSo mt = HoSoDAO.getInstance().selectById(ctpn.getMaHoSo());
                pdfTable.addCell(new PdfPCell(new Phrase(ctpn.getMaHoSo(), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(mt.getTenNguoi(), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(formatter.format(mt.getCMND()) + "đ", fontData)));
